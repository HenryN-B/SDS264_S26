...
The output of the loop will be a vector of doubles. We have then for the length of numbers in means we generate a random number then making n number from a normal distribution with a mean of i and contatanates with output each time. This gives one list of random length that has random numbers with means 0, then 1, then 2.

The second one is a bit diffrent where instead of putting the numbers into the same vector we have 3 sub vectors for each of the i values for the means.
