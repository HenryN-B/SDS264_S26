...
The whole tibble/datafram is now one row shorter becasue since there was a NA in a it got rid of all of other columns in that row too.
...
This is because the values of 1 2 and 3 get recycled so the 6 has much more weight.
...
wt_mean <- function(x, w) {
  if (length(x) != length(w)) {
    stop("`x` and `w` must be the same length", call. = T)
  } else {
  sum(w * x) / sum(w)
  }  
}
...
This gives us the traceback to what line caused the error 
...
This is because is.na when done a vector checks each value if it is NA and returns a vector, so the if statement will error because result of is.na is a vector and we need a single value.
...
This will create a table of a data frame filtered by come condition which in our case in a few manufacturer.
it then counts the number of a chosen var, in our case its manufacturer, and then sort or does not sort based on what the user inputs with a default of True. It then creates a prop variable. 
...
The function creates a table by counting all combinations of the rows and cols variables, then pivoting the cols values into column headers. For this example, it produces a table where each row is a manufacturer model pair, each column is a cylinder count.
...
summarise(mean_hw = mean(hwy),
          se_hwy = standard_error(hwy),
          mean_cty = mean(cty),
          se_cty = standard_error(cty))
```
...
```{r}
se_na <- function(x, includeNAs = FALSE) {
  if (includeNAs) {
    x <- na.omit(x)
  } 
...
flights |>
  ...
  summarise(mean_arr_delay = mean(arr_delay, na.rm = T),
            se_arr_delay = se_na(arr_delay, T),
            mean_dep_delay = mean(dep_delay, na.rm = T),
            se_dep_delay = se_na(dep_delay,T))
```
...
both_na(c(1, 2, 3, NA, NA),c(NA, 1, 2, 3, NA))
both_na(c(1,4,NA,7,NA,9,10), c(NA,4,NA,7,NA,9,10))
...
We get a warning because they are different lengths. We can fix this by adding stop.
...
We can see that it is necessary because there is more flights missing arival delays then missing departure delays. 
...
minutes_between <- function(time1, time2) {
  hour1 <- time1 %/% 100
  min1 <- time1 %% 100
  hour2 <- time2 %/% 100
  min2 <- time2 %% 100
...
area_inch_from_cm <- function(lengthcm, widthcm) {
  (lengthcm / 2.54) * (widthcm / 2.54)
}

non_answer_collapse <- function(x) {
...
This function takes in a vector x, which is a vector of numbers.
```{r}
x <- c(3,9,8,2,4)
```

Then the function takes `x[-1]` which is the vector x without the first element, and `x[1:(length(x) - 1)]` which gives all elements but the last.
...
We then take the difference of the 2 parts of the vector 
```{r}
diff <- x[-1] - x[1:(length(x) - 1)]
```

We then sum the number of element of diff that are less then 0.
```{r}
sum(diff < 0)
```

What this is doing is it is couting the number of elments in the vector x that are smaller then the element before it in the vector. So we can see that in the array 3,9,8,2,4 it returned 2, this is becuase 8 was less then 9 and 2 is less then 8.
...
foo1_na <- function(x) {
  if(is.na(sum(x))) {
    stop("x can not have NA values", call. = FALSE)
  }
  diff <- x[-1] - x[1:(length(x) - 1)]
  sum(diff < 0)
}

foo1_na(c(1,5,6,2,NA,8,1))
foo1_na(c(8,7,6,5,4))
foo1_na(c(8,7,6,NA,5,4))
```
...
greet(Sys.time())
greet("2018-05-03 11:38:01 CDT")
greet("2018-05-03 21:38:01 CDT")
```
...
mpg |>
  summary6(hwy, na.rm = T) 

gss_cat |>
  summary6(age, na.rm = T)
...
last_index(c(1,5,3,73,2))
last_index(c("red","blue","green"))
```
...
plot_party_age <- function(data, center = mean, spread = sd) {
  title <- rlang::englue("{{center}} vs {{spread}}")
  data |>
